    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-striped table-hover datatable datatable-Permission">
                ...
                <tbody>
                    @foreach($questions_options as $key => $questions_option)
                        <tr data-entry-id="{{ $questions_option->id }}">
                            ...
                            <td>
                                {{ $questions_option->correct == 1 ? 'Yes' : 'No' }}
                            </td>
                            <td>
                                @can('question_option_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.question-options.show', $questions_option->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('question_option_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.question-options.edit', $questions_option->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('question_option_delete')
                                    <form action="{{ route('admin.question-options.destroy', $questions_option->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
